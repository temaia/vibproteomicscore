...
def WriteToExcel2(arguments_dict):
    ...
    # write header
        ...
        worksheet_s.write(3, 2, ugettext("Reference (negative control) Condition"), header) # from dropdown 'Yes' or 'No' 
     ...
     # '3-Nb_samples', '3-Isotopic_labeling', '3-Isotopic_labeling_details', 'PID'])
        ...
        print(no_replicates)
        # negative control condition(s)
        NegativeControl_conditions = arguments_dict["3-NegativeControl_conditions"][0].replace(' ','').split(',')
        # indexes from ec object that are negative controls
        NegativeControl_conditions_ecindexes = list()
        for i in range(len(ec)):
            if ec[i] in NegativeControl_conditions:
                NegativeControl_conditions_ecindexes.append(i)
...
# fill in no_replicates list in case it does not match the number of ec
        ...
        for idx in range(0,no_samples,1):
            ...
            if str(replicatelist[idx])!="": 
                ...
                worksheet_s.write_string(row, 2, "", celledit)  
            # reference condition boolean
            #worksheet_s.write_string(row, 3, 'No', celledit)
            #worksheet_s.data_validation(row, 3,row, 3, { 'validate' :'list',
            #                                    'source' : refcond})
            # sample type delivered
