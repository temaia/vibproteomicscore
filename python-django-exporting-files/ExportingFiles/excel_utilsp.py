...
def WriteToExcel(experiment_data, town=None):
    output = io.StringIO()
    ...
    # write title
    title_text = "{0} {1}".format(ugettext("Experiment_ID"), user.profile.issue)
    # merge cells
    worksheet_s.merge_range('B2:I2', title_text, title)

    # write header
    worksheet_s.write(4, 0, ugettext("Sample_Name"), header)
    worksheet_s.write(4, 1, ugettext("Condition"), header)
    worksheet_s.write(4, 2, ugettext("Isotopic label"), header)
    worksheet_s.write(4, 3, ugettext("Sample_Type"), header)
    worksheet_s.write(4, 4, ugettext("Buffer composition"), header)
    worksheet_s.write(4, 5, ugettext("Protein_Conc. (μmol/μl)"), header)
    worksheet_s.write(4, 6, ugettext("Volume (μl)"), header)
    worksheet_s.write(4, 7, ugettext("Sample_Vial"), header)
    worksheet_s.write(4, 8, ugettext("Sample_Type"), header)
